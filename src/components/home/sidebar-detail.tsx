		...
		<>
			{
				createPortal(
					<div className="fixed inset-0 z-[60] overflow-hidden">
						...
						></div>
						<aside
							className="fixed top-0 right-0 w-1/3 h-full overflow-y-auto bg-blue-dark scrollbar border-l border-l-gray-600 z-[100]"
						>
							<div className="p-6">
								<button
									type="button"
									...
									</button>
									{
										expanded.products && (
											<div className="flex flex-col gap-2 w-full items-start">
												{
													data && data.next && data.next.item && data.next.item.length > 0 && data.next.item.map((item) => (
														<CardProductDetail
															key={item.id}
															id={item.id}
